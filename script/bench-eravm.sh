...
row "function" "YulSafe" "Solady" "Plain"
row "first_deposit"      "$(gas_used $YS 'deposit(uint256,address)' $D1 $ME)" "$(gas_used $SO 'deposit(uint256,address)' $D1 $ME)" "$(gas_used $PL 'deposit(uint256,address)' $D1 $ME)"
gas_used $YS 'deposit(uint256,address)' $D2 $ME >/dev/null; gas_used $SO 'deposit(uint256,address)' $D2 $ME >/dev/null; gas_used $PL 'deposit(uint256,address)' $D2 $ME >/dev/null
row "subsequent_deposit" "$(gas_used $YS 'deposit(uint256,address)' $D2 $ME)" "$(gas_used $SO 'deposit(uint256,address)' $D2 $ME)" "$(gas_used $PL 'deposit(uint256,address)' $D2 $ME)"
row "mint"               "$(gas_used $YS 'mint(uint256,address)' $D2 $ME)" "$(gas_used $SO 'mint(uint256,address)' $D2 $ME)" "$(gas_used $PL 'mint(uint256,address)' $D2 $ME)"
row "withdraw"           "$(gas_used $YS 'withdraw(uint256,address,address)' $D2 $ME $ME)" "$(gas_used $SO 'withdraw(uint256,address,address)' $D2 $ME $ME)" "$(gas_used $PL 'withdraw(uint256,address,address)' $D2 $ME $ME)"
row "redeem"             "$(gas_used $YS 'redeem(uint256,address,address)' $D2 $ME $ME)" "$(gas_used $SO 'redeem(uint256,address,address)' $D2 $ME $ME)" "$(gas_used $PL 'redeem(uint256,address,address)' $D2 $ME $ME)"
echo "eth_estimateGas for views (includes zkSync's fixed per-transaction overhead):"
row "totalAssets"        "$(estimate $YS 'totalAssets()')" "$(estimate $SO 'totalAssets()')" "$(estimate $PL 'totalAssets()')"
row "convertToShares"    "$(estimate $YS 'convertToShares(uint256)' $D2)" "$(estimate $SO 'convertToShares(uint256)' $D2)" "$(estimate $PL 'convertToShares(uint256)' $D2)"
row "convertToAssets"    "$(estimate $YS 'convertToAssets(uint256)' $D2)" "$(estimate $SO 'convertToAssets(uint256)' $D2)" "$(estimate $PL 'convertToAssets(uint256)' $D2)"
